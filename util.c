...
{
	FILE *fp __defer_func(defered_fclose) = fopen(file_name, "rb");
	if (fp == NULL)
		errlogf(2, "failed to open file '%s'.\n", file_name);
	...
	{
		fclose(fp);
		errlogf(3, "failed to allocate the initial buffer size\n");
	}
	...
	while ((cur_chunk_read = fread(ret_buf.ptr + ret_buf.size, 1, buf_size - ret_buf.size- 1, fp)) > 0)
	{
		...
		if (ret_buf.size >= buf_size - 1)
		{
			...
			if (temp_buf == NULL)
			{
				free(ret_buf.ptr);
				fclose(fp);
				errlogf(4, "failed to reallocate buffer with size %zu\n", buf_size);
			}
